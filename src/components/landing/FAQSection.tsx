...
const FAQSection = () => (
  <section className="py-16 md:py-28 bg-secondary relative">
    <div className="absolute inset-0">
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[400px] md:w-[500px] h-[250px] md:h-[300px] bg-primary/5 rounded-full blur-[100px]" />
    </div>
      ...
      <Accordion type="single" collapsible className="space-y-2 md:space-y-3">
        {faqs.map((faq, i) => (
          <AccordionItem
            key={i}
            value={`faq-${i}`}
            className="border border-border rounded-xl px-4 md:px-6 overflow-hidden bg-card/30 backdrop-blur-sm"
          >
            <AccordionTrigger className="text-left text-sm md:text-base font-semibold text-foreground hover:text-primary transition-colors [&[data-state=open]]:text-primary py-4 md:py-5">
              {faq.q}
            </AccordionTrigger>
            <AccordionContent className="text-muted-foreground leading-[1.7] text-sm md:text-base">
              {faq.a}
            </AccordionContent>
          </AccordionItem>
        ))}
